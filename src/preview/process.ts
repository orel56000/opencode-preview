...
export class PreviewInstance {
  ...
  private setState(patch: Partial<PreviewState>): void {
    this.state = {
      ...this.state,
      ...patch,
      definition: this.definition,
      uptimeMs:
        this.startTime && patch.status === "running"
...
async function killChildren(parentPid: number): Promise<void> {
  return new Promise((resolve) => {
...
async function waitForPortRelease(port: number, timeout = 5000): Promise<void> {
