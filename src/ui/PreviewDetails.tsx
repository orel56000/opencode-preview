...
export function PreviewDetails(props: { instance: PreviewInstance }) {
  ...
  return (
    <box border={true} title="Details" style={{ padding: 1, gap: 1 }}>
      ...
      <scrollbox style={{ height: 12, borderColor: theme.borderSubtle }}>
        {logs().map((line) => (
          <text style={{ fg: theme.textMuted }}>{line}</text>
        ))}
      </scrollbox>
    </box>
  )
