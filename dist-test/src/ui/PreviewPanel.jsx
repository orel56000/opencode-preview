...
export function PreviewPanel(props) {
    const [selectedId, setSelectedId] = createSignal(undefined);
    ...
    return (<PreviewContext.Provider value={{ manager, api: props.api }}>
      <box border={true} title="PREVIEWS" gap={1} style={{ padding: 1 }}>
        {instances().length === 0 ? (<text style={{ fg: props.api.theme.current.textMuted }}>
            No previews configured.
          </text>) : (instances().map((id) => {
            const instance = manager.getInstance(id);
            if (!instance)
                return null;
            return (<PreviewItem instance={instance} selected={selectedId() === id} onSelect={() => setSelectedId(id)}/>);
        }))}
...
//# sourceMappingURL=PreviewPanel.jsx.map