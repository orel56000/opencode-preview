...
export function PreviewItem(props) {
    ...
    const urlText = createMemo(() => {
        ...
        if (s.status === "running" && s.url)
            return s.url;
        if (s.status === "port-in-use")
            return `Port ${s.definition.port}`;
        return `:${s.definition.port}`;
    });
    const theme = api.theme.current;
    return (<box border={true} style={{
            borderColor: props.selected ? theme.borderActive : theme.border,
            padding: 1,
        }}>
      <text style={{
            fg: state().status === "crashed"
                ? theme.error
      ...
      {state().status === "crashed" && state().error ? (<text style={{ fg: theme.error }}>{state().error}</text>) : null}
      {state().status === "port-in-use" && state().error ? (<text style={{ fg: theme.warning }}>{state().error}</text>) : null}
      <select options={actionOptions()} onSelect={(_index, option) => {
            if (option?.value)
                handleAction(option.value);
        }}/>
    </box>);
}
//# sourceMappingURL=PreviewItem.jsx.map